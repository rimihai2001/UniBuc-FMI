...
CV = CountVectorizer(ngram_range=(1, 6), encoding="mbcs",
...
rfc_200.fit(train_data, train_labels_language)
prediction_valid = rfc_200.predict(validation_data)
...
rfc_200 = RandomForestClassifier(n_estimators=200)
rfc_200.fit(train_data, train_labels_language)
prediction_test = rfc_200.predict(test_data)

# model ce se antreneaza cu datele de train si validation
rfc_200 = RandomForestClassifier(n_estimators=200)
rfc_200.fit(all_data, all_labels_language)
prediction = rfc_200.predict(test_data)

#afisare in CSV
pd.DataFrame(data={"id": test_samples_index, "label": prediction}).to_csv(
    "../Submission_Data/RFC_200_Submission16.csv", index=False)
